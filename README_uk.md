...
Також редактор карти представлений графічним скриптом, написаним на python kivy.  
...
Кнопка **Save Map** дозволить зберегти картку як .json файл.  
Натиснувши на один з об'єктів у списку, виберіть його.  
Посередині приблизна карта, яка має вийти у грі.  
Натиснувши на один з об'єктів на карті, ви його виберіть.  
...
3. GlobalSettings:
    ...
    - **objMaps** - карти з папки Maps
4. NTEmapManager:
    - **loadLevel(mapname: str = "globalMap")** - завантажує картку, що зберігається в папці Maps, з ім'ям "mapname"
...
5. NTEngineClasses:
    ...
    - **клас Vector3**:
        ...
        - **Vector3.sum(a, b)** - оператор "**+**" - сума векторів "a" та "b"
        - **Vector3.substr(a, b)** - оператор "**-**" - з вектора "a" віднімає вектор "b"
        ...
        - **Vector3.dot(a, b)** - оператор "**" - Скалярний добуток векторів "a" та "b"
...
3. **клас Camera**:
    - **offset** - зміщення камери від лівого кута, спочатку дорівнює половині розміру карти, що відмальовується.
4. **клас BoxCollider**:
