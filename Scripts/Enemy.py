...
class Enemy(Behavior):
    ...
    def onCollide(self, collider: Transform):
        if isinstance(collider.beh, Scripts.Player.Player):
            ObjList.getObj(instantiate(Scripts.FireBall.FireBall,
                                       Vec3(self.gameobject.tr.position.x - 1, self.gameobject.tr.position.y))).dir = Vec3(
                -1, 0)
            ObjList.getObj(instantiate(Scripts.FireBall.FireBall,
                                       Vec3(self.gameobject.tr.position.x + 1, self.gameobject.tr.position.y))).dir = Vec3(
                1, 0)
            ObjList.getObj(instantiate(Scripts.FireBall.FireBall,
                                       Vec3(self.gameobject.tr.position.x, self.gameobject.tr.position.y - 1))).dir = Vec3(
                0, -1)
            ObjList.getObj(instantiate(Scripts.FireBall.FireBall,
                                       Vec3(self.gameobject.tr.position.x, self.gameobject.tr.position.y + 1))).dir = Vec3(
                0, 1)
